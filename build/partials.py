"""Shared chrome for the Monga Brothers site.

Every page is plain static HTML so it can be served from any host with no
build step at all. This module only exists so the header, footer and other
repeated blocks are written once instead of eight times; running
``python3 build/build.py`` re-stamps them into the page templates in
``build/pages/`` and writes the final HTML to the repository root.
"""
...
ICONS = {
    "globe": '<path d="M12 2a10 10 0 1 0 0 20 10 10 0 0 0 0-20Zm6.9 9h-2.95a15.7 15.7 0 0 0-1.2-5.35A8.02 8.02 0 0 1 18.9 11ZM12 4.04c.83 1.2 1.71 3.3 1.93 6.96h-3.86C10.29 7.34 11.17 5.24 12 4.04ZM4.26 13h2.95c.13 2.02.55 3.87 1.2 5.35A8.02 8.02 0 0 1 4.26 13Zm2.95-2H4.26a8.02 8.02 0 0 1 4.15-5.35A15.7 15.7 0 0 0 7.21 11Zm4.79 8.96c-.83-1.2-1.71-3.3-1.93-6.96h3.86c-.22 3.66-1.1 5.76-1.93 6.96Zm3.59-.61c.65-1.48 1.07-3.33 1.2-5.35h2.95a8.02 8.02 0 0 1-4.15 5.35Z"/>',
    "phone": '<path d="M17 1H7a2 2 0 0 0-2 2v18a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V3a2 2 0 0 0-2-2Zm-5 21a1.4 1.4 0 1 1 0-2.8 1.4 1.4 0 0 1 0 2.8Zm5-4H7V4h10Z"/>',
    ...
    "users": '<path d="M16 11a4 4 0 1 0-4-4 4 4 0 0 0 4 4Zm-8 1a3 3 0 1 0-3-3 3 3 0 0 0 3 3Zm0 2c-2.33 0-7 1.17-7 3.5V20h7v-2.5A4.5 4.5 0 0 1 9.3 14.1 12 12 0 0 0 8 14Zm8 0c-2.67 0-8 1.34-8 4v2h16v-2c0-2.66-5.33-4-8-4Z"/>',
...
def icon(name, cls=""):
    ...
    return (
        '<svg%s viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">%s</svg>' % (c, body)
    )
...
<meta property="og:image" content="assets/img/poster-drop-forging.jpg">
<link rel="icon" href="assets/img/favicon.svg" type="image/svg+xml">
<link rel="preconnect" href="https://fonts.googleapis.com">
<link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
<link href="https://fonts.googleapis.com/css2?family=Barlow+Condensed:wght@600;700&family=Poppins:wght@400;500;600;700&display=swap" rel="stylesheet">
<link rel="stylesheet" href="assets/css/style.css">{css_extra}
</head>
<body>
"""
...
def header(current=""):
    ...
    for label, href, sub in NAV:
        ...
        if sub:
            links = "".join(
                '<li><a href="%s">%s</a></li>' % (h, t) for t, h in sub
            )
            items.append(
                f'<li{cur}><a href="{href}" data-submenu>{label}'
...
<header class="topbar">
  <div class="shell topbar__inner">
    <a class="logo" href="index.html">
      <img src="assets/img/logo-monga-brothers.png" width="285" height="60"
           alt="Monga Brothers Ltd. — an ISO 9001:2015 company">
    </a>
    <div class="topbar__facts">
      <div class="fact">
        <span class="fact__icon">{icon('pin')}</span>
        <span><b>Address</b>{c['address_1']}<br>{c['address_2']}</span>
      </div>
      <div class="fact">
        <span class="fact__icon">{icon('phone')}</span>
        <span><b>Call us</b><a href="tel:{c['phone_href']}">{c['phone']}</a><br>
        <a href="mailto:{c['email']}">{c['email']}</a></span>
      </div>
    </div>
  </div>
</header>
...
<div class="navbar">
  <div class="shell navbar__inner">
    <button class="nav-toggle" type="button" aria-expanded="false" aria-controls="primary-nav">
      <span class="nav-toggle__bars"><i></i><i></i><i></i></span> Menu
    </button>
    <ul class="nav" id="primary-nav">{nav_html}</ul>
    <a class="navbar__cta" href="contact.html">{icon('arrow', 'nav__caret')} Get a quote</a>
  </div>
...
def ticker(words):
    ...
    return f"""<div class="ticker" aria-hidden="true"><div class="ticker__track">{spans}</div></div>"""


def cta_band(
    title="Ready to move your project forward?",
    text="Send us a drawing or a specification — we will come back with feasibility, material options and lead time.",
):
  ...
  <div class="shell cta__inner">
    ...
    <a class="btn btn--dark" href="contact.html" data-reveal="right">Request a quotation {icon('arrow', 'nav__caret')}</a>
  </div>
</section>
"""
...
def footer():
  ...
  <div class="shell">
    <div class="footer__grid">
      <div>
        <img class="logo__img" src="assets/img/logo-monga-brothers-light.png" width="285" height="60"
             alt="Monga Brothers Ltd." loading="lazy">
        <p style="margin-top:1.2rem">Driving forward with precision engineering, unmatched
        reliability and innovative solutions for a safer, more connected world. In-house
        capability from scrap metal to finished component.</p>
        <div class="socials">
      ...
      <div>
        ...
        <ul>
          ...
          <li><a href="manufacturing.html">Our facilities</a></li>
          <li><a href="industries.html">Service areas</a></li>
