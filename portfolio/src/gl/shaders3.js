...
uniform float uReveal;     // 0..1 the line draws itself from 2020 forward
uniform float uActiveU;    // continuous position of the active year, 0..6
uniform float uHeat;
// sized to YEARS.length in layout3.js; chrono.js clamps its upload to match
uniform vec2  uNodes[7];
uniform float uNodeIn[7];  // per-node materialisation
out vec4 frag;
...
void main(){
  ...
  // --- the rail -----------------------------------------------------------
  // u runs 0..1 from the 2020 end to the 2026 end
