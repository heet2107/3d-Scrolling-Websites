...
/** How much of the frame's width the active plate is allowed to take. */
const FIT = 0.56;
const FIT_PORTRAIT = 0.88;
...
export function deckConfig(aspect, portrait) {
  ...
  const step = z0 * (portrait ? 0.60 : 0.78);
  const snake = z0 * (portrait ? 0.42 : 0.80);
  ...
  return {
    ...
    shift: portrait ? [0, 0.40] : [-0.30, 0.05],
...
export function pose(d, cfg, out = {}) {
  ...
  const zd = d >= 0 ? d : d * 0.55;
  const past = Math.max(0, -d);

  out.z = -(cfg.z0 + zd * cfg.step);
  out.x = cfg.snake * Math.sin(d * 0.62) * (1 + past * 0.5);
  out.y = cfg.baseY + cfg.z0 * 0.028 * Math.sin(d * 0.85) + past * cfg.z0 * 0.10;
  ...
  out.ry = -0.60 * Math.atan2(out.x, dist);
  out.rx = 0.030 * Math.sin(d * 0.7);
  out.rz = 0.016 * Math.sin(d * 1.1 + 0.6) - past * 0.14;

  out.focus = Math.exp(-d * d * 1.30);
  out.alpha = smoothstep(-1.15, -0.42, d)
...
export function visible(d, cfg) {
  return d > -1.2 && d < cfg.vis && -(cfg.z0 + (d >= 0 ? d : d * 0.55) * cfg.step) < -0.35;
}
